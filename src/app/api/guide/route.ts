...
function allowed(userId: string) { const now = Date.now(); const current = windows.get(userId); if (!current || now - current.startedAt > 60_000) { windows.set(userId, { startedAt: now, count: 1 }); return true; } if (current.count >= 12) return false; current.count += 1; return true; }
...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: process.env.GROQ_MODEL || "groq/compound-mini", temperature: 0.3, max_tokens: 360, messages: [{ role: "system", content: system }, ...parsed.data.messages] }) });
