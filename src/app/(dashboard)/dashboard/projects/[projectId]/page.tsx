...
import { getPortalFeedbackForProject } from "~/services/portal-feedback";
...
export default async function ProjectDetailPage({ params, searchParams }: { params: Promise<{ projectId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [clients, members, addableMembers, tasks, milestones, finance, invoices, timeline, attachments, clientFeedback] = await Promise.all([
    ...
    getProjectActivity(projectId, organization.id), getAttachmentsForProject(projectId, organization.id), getPortalFeedbackForProject(projectId, organization.id),
  ]);
  ...
  return <div className="space-y-7">
    ...
    {activeTab === "overview" && <div className="space-y-8"><Milestones projectId={projectId} milestones={milestones} canManage={canEdit} />{clientFeedback.length > 0 && <section className="worknest-panel rounded-2xl border border-neutral-200 bg-white p-5"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#d92d27]">Client voice</p><h2 className="mt-1 font-bold text-neutral-900">Recent feedback</h2></div><span className="rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-bold text-neutral-500">{clientFeedback.length}</span></div><div className="mt-4 space-y-2">{clientFeedback.slice(0, 5).map((item) => <div key={item.id} className="rounded-xl bg-neutral-50 p-3"><div className="flex items-center justify-between"><span className={`text-[10px] font-bold uppercase tracking-wide ${item.type === "approval" ? "text-emerald-700" : "text-neutral-500"}`}>{item.type === "approval" ? "Client approval" : "Client feedback"}</span><time className="text-xs text-neutral-400">{new Date(item.createdAt).toLocaleString()}</time></div><p className="mt-1 text-sm text-neutral-700">{item.message}</p></div>)}</div></section>}<section className="grid gap-6 lg:grid-cols-2"><div><h2 className="text-sm font-medium text-neutral-900">Project details</h2><div className="mt-2"><ProjectEditForm project={project} clients={clients} canEdit={canEdit} /></div></div><div><h2 className="text-sm font-medium text-neutral-900">Members</h2><div className="mt-2"><ProjectMembers projectId={project.id} members={members} addableMembers={addableMembers} canManage={canManageMembers} /></div></div></section></div>}
