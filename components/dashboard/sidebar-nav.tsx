...
export default function SidebarNav({ items }: { items: Item[] }) {
  ...
  return <nav className="mt-5 flex gap-1 overflow-x-auto lg:mt-8 lg:block">{items.map((item, index) => {
    const active = item.href === "/dashboard" ? pathname === item.href : pathname.startsWith(item.href);
    const showSection = item.section && (index === 0 || item.section !== items[index - 1]?.section);
    return <div key={item.href} className="lg:mt-1">{showSection && <p className="mb-2 mt-6 px-3 text-[10px] font-bold uppercase tracking-[0.2em] text-neutral-600">{item.section}</p>}<Link href={item.href} className={`group flex shrink-0 items-center gap-3 rounded-2xl px-3 py-2 text-sm font-semibold transition-all duration-200 ${active ? "bg-[#d92d27] text-white shadow-lg shadow-red-950/30" : "text-neutral-400 hover:bg-white/[0.07] hover:text-white"}`}><Mark icon={item.icon}/><span>{item.label}</span>{item.badge ? <span className={`ml-auto rounded-full px-2 py-0.5 text-[10px] ${active ? "bg-white/20" : "bg-white/10 text-neutral-300"}`}>{item.badge}</span> : null}</Link></div>;
  })}</nav>;
}
