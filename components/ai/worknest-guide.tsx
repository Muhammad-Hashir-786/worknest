...
export default function WorkNestGuide() {
  ...
  return <div className="fixed bottom-3 right-3 z-50 flex max-w-[calc(100vw-1.5rem)] flex-col items-end sm:bottom-5 sm:right-5">
    <div className={`origin-bottom-right transition duration-200 ${open ? "scale-100 opacity-100" : "pointer-events-none scale-95 opacity-0"}`} aria-hidden={!open}>
      <section id="worknest-guide" aria-label="WorkNest Guide" className="mb-3 flex h-[min(620px,calc(100dvh-5.5rem))] w-[min(460px,calc(100vw-1.5rem))] flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-2xl shadow-neutral-900/25">
    ...
    <button type="button" onClick={() => { setOpen((value) => !value); window.setTimeout(() => inputRef.current?.focus(), 200); }} aria-expanded={open} aria-controls="worknest-guide" aria-label="Open WorkNest Guide" className="flex h-12 items-center gap-2 rounded-full bg-[#d92d27] px-4 text-sm font-bold text-white shadow-xl shadow-red-950/30 transition hover:-translate-y-0.5 hover:bg-[#b42318] sm:h-14 sm:px-5"><span className="grid h-7 w-7 place-items-center rounded-full bg-white/15 text-base">✦</span>{open ? "Close guide" : "Ask WorkNest"}</button>
  </div>;
}
