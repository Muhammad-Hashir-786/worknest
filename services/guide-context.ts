...
export async function getGuideWorkspaceContext(organizationId: string, question: string, userId?: string): Promise<string> {
  await connectDB();
  ...
  const wantsOverdue = /overdue|late|past due|behind/i.test(question);
  const wantsMine = /\b(me|my|assigned to me)\b/i.test(question);
  const overdueMine = userId && (wantsOverdue || wantsMine)
    ? await Task.find({ organization: organizationId, assignee: userId, dueDate: { $lt: now }, status: { $ne: "completed" } })
      .select("_id title status priority dueDate project").populate("project", "name").sort({ dueDate: 1 }).limit(30).lean()
    : [];
  const overdueLines = overdueMine.map((task) => {
    const project = task.project as unknown as { _id?: { toString(): string }; name?: string } | null;
    return `- ${task.title} | ${project?.name ?? "Unknown project"} | ${task.status} | due ${task.dueDate?.toISOString().slice(0, 10) ?? "not set"} | link: ${project?._id ? `/dashboard/projects/${project._id.toString()}/tasks/${task._id}` : "/dashboard/projects"}`;
  }).join("\n");
  const blockerHint = /block|stuck|health|summarize/i.test(question)
    ? "For blockers, treat overdue incomplete tasks and incomplete dependency chains as blockers. Do not infer a blocker when there is no evidence."
    : "";
  return `Today is ${date}. This is private, current data from the signed-in user's active organization. Use it to answer factual questions.\n${blockerHint}\n\nMATCHES FOR THE USER'S QUESTION:\n${matchLines || "None"}\n\nOVERDUE TASKS ASSIGNED TO THE CURRENT USER:\n${overdueLines || "None"}\n\nPROJECTS:\n${projectLines || "None"}\n\nTASKS:\n${taskLines || "None"}\n\nINVOICES:\n${invoiceLines || "None"}`.slice(0, 30000);
}
